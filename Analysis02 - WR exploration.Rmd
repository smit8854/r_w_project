...
ggplot(
  ...
  aes(x=f3f2, color=tran))+geom_density(size=2)+
...
```{r}
ggplot(data=(rw.merge %>% filter(!is.na(target), tran == c("r", "w"))), aes(x=rating, color=tran))+geom_density(size=2)+
scale_colour_manual(name = "Transcription", values=c("#7a0019", "#ffcc33")) + 
scale_y_continuous(breaks=NULL, name=NULL) +
scale_x_continuous(breaks=seq(0,1,.2), name="Listener Rating: 0=R 1=W") +
theme_tufte()
```
