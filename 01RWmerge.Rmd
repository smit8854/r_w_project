...
ChildID was created in excel. To fully smooth the transition from raw PRAAT to R, write code to do that here. 


# Import Files

```{r}
vas <- read.csv("C:\\Users\\tenni\\OneDrive\\Documents\\GitHub\\r_w_project\\VAS.csv")
```
one record per rater per sound


```{r}
acous <- read.csv("C:\\Users\\tenni\\OneDrive\\Documents\\GitHub\\r_w_project\\acoustic.csv")
```
